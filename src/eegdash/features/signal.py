...
def signal_katz_fractal_dim(x, eps=1e-7):
    ...
    a = dists.mean(axis=-1)
    log_n = np.log(L / a)
    d = np.abs(x[:, 1:].T - x[:, 0]).T.max(axis=-1)
...
class EntropyFeatureExtractor(ByChannelFeatureExtractor):
    def preprocess(self, x, m=2, r=0.2, l=1):
        rr = r * x.std(axis=-1)
        counts_m = np.empty((x.shape[0], (x.shape[-1] - m + 1) // l))
        counts_mp1 = np.empty((x.shape[0], (x.shape[-1] - m) // l))
        for i in range(x.shape[0]):
